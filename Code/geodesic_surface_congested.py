...
def geodesic(nTime, nameFileD, mub0,mub1, cCongestion, eps, Nit, detailStudy) :
	...
	cCongestion: constant for the intensity of the regularization (alpha in the article)
	eps: regularization parameter for the linear system inversion 
	Nit: Number of iterations 
	detailStudy: True if the value of the objective functional (i.e. the Lagrangian) and the residuals are computed at every time step (slow), false if computed every 10 iterations (fast)
	
	Output.
	phi,mu,A,E,B: values of the relevant quantities in the interpolation, cf. the article for more details (Note: E is the momentum, denoted by m in the article)
	objectiveValue: evolution (in term of the number of iterations of the ADMM) of the objective value, ie the Lagrangian 
	primalResidual,dualResidual: evolution (in term of the number of iterations of the ADMM) of primal and dual residuals, cf the article for details of their computation. 
	"""
	...
	gradientDMatrix, divergenceDMatrix, LaplacianDMatrix = surface_pre_computations.geometricMatrices(Vertices, Triangles, Edges, areaTriangles, angleTriangles, baseFunction )
	...
	# Vectorized arrays
	areaVectorized = np.kron( np.kron( np.ones( 6*nTime ), areaTriangles  ), np.ones(3)).reshape( nTime,2, 3,nTriangles,3 )
	areaVerticesGlobal = np.kron( np.ones(nTime), areaVertices ).reshape((nTime,nVertices))
	areaVerticesGlobalStaggerred = np.kron( np.ones(nTime+1), areaVertices ).reshape((nTime+1,nVertices))
	...
	geomDic = {'nTime': nTime, 'DeltaTime':DeltaTime, 'Vertices':Vertices, 'Triangles':Triangles, 'Edges':Edges, 'areaTriangles':areaTriangles, 'gradientDMatrix':gradientDMatrix, 'divergenceDMatrix':divergenceDMatrix, 'LaplacianDMatrix':LaplacianDMatrix, 'originTriangles':originTriangles, 'areaVertices':areaVertices, 'vertexTriangles':vertexTriangles,'nVertices':nVertices, 'nTriangles':nTriangles, 'nEdges':nEdges, 'areaVerticesGlobal':areaVerticesGlobal, 'areaVectorized':areaVectorized} 
	...
	# Centered in Time, the second component is indicating on which side of the temporal it comes from. Third component indicates the origine of the triangle on which it is. Fourth component is the triangle. Last component corresponds to the fact that we are looking at a vector of R^3.  
	...
	# Lagrange multiplier associated to the congestion. If there is no congestion, the value of this parameter will always stay at 0.   
	...
	for counterMain in range(Nit) :
		...
		bSquaredArray = originTrianglesGlobal.dot(squareAux.reshape(nTime*3*nTriangles)).reshape((nTime,nVertices))
		...
		projDiscriminating = (np.greater( projObjective, 10**(-16) * np.ones( (nTime,nVertices) ) )).astype(float)
		...
		while np.max(projObjective) > 10**(-10) and counterProj < 20 : 
			...
			projObjective = toProjectA + 6*(1. + cCongestion * r) *xProj + np.divide(bSquaredArray,  np.square( 1 - xProj ) ) 
			# Derivative of the ojective functional 
			dProjObjective = 6*(1. + cCongestion * r) - 2.* np.divide( bSquaredArray, np.power( xProj - 1 , 3) ) 
			# Update of xProj 
		...
		xProjTriangles = np.kron(vertexTrianglesGlobal.dot( xProj.reshape(nTime*nVertices)  ).reshape((nTime,3,nTriangles)), np.ones(3)).reshape((nTime,3,nTriangles,3))
		...
		print(  str(counterProj) + " iterations needed; error committed: " + str(np.max(projObjective)) + ".")
		...
		if isCongestion :
			print("Congestion") 
			print( scalarProductFun(lambdaC, mu, geomDic ) - 1/(2.*cCongestion) * scalarProductFun( lambdaC, np.multiply(lambdaC, areaVerticesGlobal/3.) , geomDic)   )
			print( cCongestion/2. * np.sum( np.divide(np.square( mu ), 1/3. * areaVertices  )  ) / nTime ) 
	...
	print( "Total time taken by the computation of the geodesic: " + str( round( endProgramm - startImport, 2) ) + "s." )
...
def gradientD(input,geomDic) : 
	...
	outputAux = geomDic['gradientDMatrix'].dot(input.transpose()).transpose().reshape((geomDic['nTime']+1,geomDic['nTriangles'],3))
...
#********************************************************************************************************				
def objectiveFunctional(phi,mu,A,E,B,lambdaC,BT,geomDic,r,cCongestion,isCongestion) : 
	"""
