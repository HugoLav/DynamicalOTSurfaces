...
parser.add_argument("-m", "--mesh", required=True, help="relative path to mesh file")
parser.add_argument(
    "-b", "--boundary", required=True, help="relative path to boundary conditions file"
)
parser.add_argument("-o", "--output", default="output.log", help="path to output file")
parser.add_argument(
    "--alpha", default=0.0, type=float, help="value of congestion parameter"
)
parser.add_argument(
    "--ntime", default=31, type=int, help="number of time steps for discretization"
)
parser.add_argument(
    "--niter", default=1000, type=int, help="maximum number of ADMM iterations"
)
parser.add_argument(
    "--tol",
    default=1e-4,
    type=float,
    help="acceptable tolerance for primal/dual feasibility",
)
parser.add_argument(
    "-v", "--verbose", action="store_true", help="print intermediate costs"
)
...
phi, mu, A, E, B, objectiveValue, primalResidual, dualResidual = geodesic(
    nTime, nameFileD, mub0, mub1, cCongestion, eps, Nit, detailStudy, verbose, tol
)
