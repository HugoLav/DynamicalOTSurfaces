...
def buildLaplacianMatrix(geomDic, eps) :
	...
	diagTimeMOHVectorized = np.kron( diagTimeMOH, np.ones(nVertices) ).reshape((nTime+1,nVertices))
	...
	LaplacianTimeMatrixModified = np.dot( np.diag(diagTimeMOH), np.dot( LaplacianTimeMatrix, np.diag(diagTimeMOH)))	
	...
	# For each eigenvalue lambda_i, listFactor[i] contains a method to solve (-lambda_i Id + Laplacian_D)x = b.   
	listFactor = []
	...
	def LaplacianInvert(input) : 
		...
		return np.multiply(  diagTimeMOHVectorized, LaplacianAuxInvert(  np.multiply(  input, diagTimeMOHVectorized )))
			
	endFact = time.time()

	print( "Factorizing the Laplace matrix: " + str( round( endFact - startFact, 2) ) + "s." )
